  ...
  return (
    <header className="flex justify-between items-center bg-brand-gray p-2 sm:p-4 rounded-xl border border-brand-light-gray">
      ...
      <div className="flex items-center gap-1 sm:gap-2 md:gap-4">
        <button onClick={handleToggleMute} className="text-brand-text hover:text-white transition-all duration-200 p-2 rounded-full hover:bg-brand-light-gray transform hover:scale-110">
          {muted ? <SoundOffIcon className="w-6 h-6" /> : <SoundOnIcon className="w-6 h-6" />}
        </button>
        {address && (
            <button onClick={handleAchievementsClick} className="text-brand-text hover:text-white transition-all duration-200 p-2 rounded-full hover:bg-brand-light-gray transform hover:scale-110">
