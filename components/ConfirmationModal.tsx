...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  const isSuccess = bettingStep === 'success';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      {/* Backdrop */}
      <div className="fixed inset-0 bg-black bg-opacity-75 transition-opacity animate-fade-in" aria-hidden="true" onClick={isSuccess ? undefined : onClose}></div>
      ...
      <div className="relative bg-brand-gray border border-brand-light-gray rounded-xl shadow-lg transform transition-all animate-scale-in w-full max-w-md p-6 sm:p-8">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-bold text-white" id="modal-title">
            {isSuccess ? 'Success' : 'Confirm Your Bet'}
          </h3>
          <button onClick={onClose} disabled={isProcessing || isSuccess} className="text-gray-400 hover:text-white text-3xl leading-none disabled:opacity-50">&times;</button>
        </div>
        
        {isSuccess ? (
          <div className="text-center py-8 flex flex-col items-center justify-center">
            <SuccessIcon className="w-24 h-24 mb-4" />
            <h4 className="text-2xl font-bold text-brand-green">Bet Placed!</h4>
            <p className="text-brand-text mt-2">Your bet is now live. Good luck!</p>
          </div>
        ) : (
